-- Pessoa como dimensao em orcamentos, metas e investimentos (null = casa toda).
ALTER TABLE public.budgets ADD COLUMN IF NOT EXISTS person TEXT NULL;
ALTER TABLE public.goals ADD COLUMN IF NOT EXISTS person TEXT NULL;
ALTER TABLE public.investments ADD COLUMN IF NOT EXISTS person TEXT NULL;

-- Um orcamento por categoria E pessoa (antes era so por categoria).
ALTER TABLE public.budgets DROP CONSTRAINT IF EXISTS budgets_user_id_category_id_key;
CREATE UNIQUE INDEX IF NOT EXISTS budgets_user_category_person_key
  ON public.budgets (user_id, category_id, COALESCE(person, ''));

-- Renomear pessoa tambem atualiza esses tres lugares.
...
BEGIN
  ...
  UPDATE budgets SET person = p_new WHERE user_id = uid AND person = p_old;
  UPDATE goals SET person = p_new WHERE user_id = uid AND person = p_old;
  UPDATE investments SET person = p_new WHERE user_id = uid AND person = p_old;
